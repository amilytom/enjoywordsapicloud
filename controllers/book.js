...
function info(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.params, ["bid"], cb);
    },
    // 查询方法，依赖校验参数方法
    query: [
      "checkParams",
      (results, cb) => {
        // 使用admin的model中的方法查询
        BookModel.findByPk(req.params.bid, {})
          .then(function (result) {
            // 查询结果处理
            // 如果查询到结果
            if (result) {
...
function add(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.body, ["bname", "classid", "cover"], cb);
    },
    // 添加方法，依赖校验参数方法
    add: (cb) => {
...
function update(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.body, ["bid", "bname", "classid", "cover"], cb);
    },
    // 更新方法，依赖校验参数方法
    update: (cb) => {
      // 使用admin的model中的方法更新
...
function remove(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.body, ["bid"], cb);
    },
    remove: (cb) => {
      // 使用admin的model中的方法更新
      BookModel.destroy({
