  ...
  let tasks = {
    ...
    query: [
      ...
      (results, cb) => {
        ...
        if (req.query.bookid) {
          whereCondition.bookid = req.query.bookid; //精确查询
          //whereCondition.bname = { [Op.like]: `%${req.query.bname}%` }; //模糊查询
        }
        if (req.query.wordid) {
          whereCondition.wordid = req.query.wordid; //精确查询
          //whereCondition.bname = { [Op.like]: `%${req.query.bname}%` }; //模糊查询
        }
        // 通过offset和limit使用model去数据库中查询，并按照创建时间排序
        WordbookModel.findAndCountAll({
...
function info(req, res) {
  ...
  let tasks = {
    ...
    query: [
      ...
      (results, cb) => {
        // 使用admin的model中的方法查询
        WordbookModel.findByPk(req.params.id, {})
          .then(function (result) {
            // 查询结果处理
            // 如果查询到结果
            if (result) {
              ...
              cb(Constant.WORDBOOK_NOT_EXSIT);
            }
          })
          .catch(function (err) {
            // 错误处理
...
function add(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.body, ["bookid", "wordid"], cb);
    },
    // 添加方法，依赖校验参数方法
    add: (cb) => {
      // 使用admin的model中的方法插入到数据库
      WordbookModel.create({
...
function update(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.body, ["id", "bookid", "wordid"], cb);
    },
    // 更新方法，依赖校验参数方法
    update: (cb) => {
      // 使用admin的model中的方法更新
      WordbookModel.update(
        {
          bookid: req.body.bookid,
          wordid: req.body.wordid,
        },
