  ...
  let tasks = {
    ...
    query: [
      ...
      (results, cb) => {
        ...
        let whereCondition = {};
        // 如果查询班级名存在，查询对象增加班级名
        if (req.query.posname) {
          //whereCondition.posname = req.query.posname; //精确查询
          whereCondition.posname = { [Op.like]: `%${req.query.posname}%` }; //模糊查询
        }
        if (req.query.dropList) {
          searchOption = {
        ...
        SpeechModel.findAndCountAll(searchOption)
...
function add(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.body, ["pos", "speech", "posname", "type"], cb);
    },
    // 添加方法，依赖校验参数方法
    add: (cb) => {
...
function remove(req, res) {
  ...
  let tasks = {
    ...
    checkParams: (cb) => {
      // 调用公共方法中的校验参数方法，成功继续后面操作，失败则传递错误信息到async最终方法
      Common.checkParams(req.body, ["pid"], cb);
    },
    remove: (cb) => {
      // 使用admin的model中的方法更新
      SpeechModel.destroy({
